 ...
 * Simulation of the SocialSway platform specifically for the Clothing & Fashion industry.
 *
 *
 *
 */
...
public class FashionSimulationSocialSway {
    ...
    public static void main(String[] args) {
        ...
        objTEST.readData(bidDataTEST, adGroupDataTEST, "/Users/bhaskarravi/Desktop/SimulationDBs/Fashion_BiddingSim_TEST.csv");
        ...
        //Header
        System.out.println("CAMPAIGN VERTICAL: FASHION");
        System.out.println("CAMPAIGN GOAL: 3500 Page Likes");
        ...
        int likesGoal = 3500;   //Campaign Goal
        ...
        while(totalLikes < likesGoal){
            ...
            if(hour < 12){
                ...
                if(likesReg.predict(hour+1) > 25 && likes[hour - 1] > 9){ //9 is the mean # of likes for the training set, 36 is the average of the high dist.
                    //MAKE CPA ONLY BID
                    timeList.add(adGroupDataTEST.get(hour).getTimeStamp());
                    impressionsList.add(adGroupDataTEST.get(hour).getUniqueImpressions());
                    clicksList.add(adGroupDataTEST.get(hour).getUniqueClicks());
                    ...
                    if((impressionsReg.predict(hour+1) * costPerThousand[hour]) / 1000 > (clicksReg.predict(hour+1) * costPerClick[hour]) || likes[hour - 1] > 9){
                        //BID CPC NOT CPM
                        timeList.add(adGroupDataTEST.get(hour).getTimeStamp());
                        impressionsList.add(adGroupDataTEST.get(hour).getUniqueImpressions());
                        clicksList.add(adGroupDataTEST.get(hour).getUniqueClicks());
