 ...
 * SocialSway Simulation for the Entertainment Industry
 * AVERAGE CPM: $0.78
 * AVERAGE CPC: $0.26
 * AVERAGE CPA: $0.56
 * AVERAGE CTR: 0.44%
 * AVERAGE PAGE LIKE RATE: 0.03%
 */
public class EntertainmentSimulationSocialSway {
    ...
    public static void main(String[] args) {
        ...
        objTEST.readData(bidDataTEST, adGroupDataTEST, "/Users/bhaskarravi/Desktop/Entertainment_BiddingSim_TEST.csv");
        ...
        //Header
        System.out.println("CAMPAIGN VERTICAL: Entertainment");
        ...
        int likesGoal = 2000;   //Campaign Goal
        ...
        while(totalLikes < likesGoal){
            ...
            if(hour < 12){
                ...
                if(likesReg.predict(hour+1) > 12 && likes[hour - 1] > 2){ //2 is the mean # of likes for the training set, 12 is the average of the high dist.
                    //MAKE CPA ONLY BID
                    timeList.add(adGroupDataTEST.get(hour).getTimeStamp());
                    impressionsList.add(adGroupDataTEST.get(hour).getUniqueImpressions());
                    clicksList.add(adGroupDataTEST.get(hour).getUniqueClicks());
                    ...
                    if((impressionsReg.predict(hour+1) * costPerThousand[hour]) / 1000 > (clicksReg.predict(hour+1) * costPerClick[hour]) || likes[hour - 1] > 2){
                        //BID CPC NOT CPM
                        timeList.add(adGroupDataTEST.get(hour).getTimeStamp());
                        impressionsList.add(adGroupDataTEST.get(hour).getUniqueImpressions());
                        clicksList.add(adGroupDataTEST.get(hour).getUniqueClicks());
                        ...
                        System.out.printf("%-10.2f", costPerAction[hour]);
                        System.out.printf("%-15.0f", (double)impressionsList.get(hour));
                        System.out.printf("%-10.0f", (double)impressionsList.get(hour)*0.0044);
                        System.out.printf("%-10.0f", (double)impressionsList.get(hour)*0.0003);
                        System.out.printf("%10.2f\n", hourlyCost);
